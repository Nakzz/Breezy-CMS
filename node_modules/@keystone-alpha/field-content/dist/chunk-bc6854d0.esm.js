...
import { c as getSelectionReference } from './chunk-ec3635e1.esm.js';
...
import { type } from './chunk-be6a2a7f.esm.js';
import { Popper } from 'react-popper';
import { b as useWindowSize, c as useScrollListener, a as useStateWithEqualityCheck } from './chunk-ffc6d47c.esm.js';
...
function markPlugin(type$$1, options) {
  return {
...
let plugins = Object.entries(marks).map(_ref => {
  ...
  return markPlugin(type$$1, options);
});

let AddBlock = _ref => {
  let editorState = _ref.editorState,
      editor = _ref.editor,
      blocks = _ref.blocks;
  ...
  return createPortal(jsx(Fragment, null, jsx("div", {
    ...
    let Sidebar = blocks[key].Sidebar;
...
function InnerToolbar(_ref) {
  ...
  }), Object.keys(blocks).map(type$$1 => {
    let ToolbarElement = blocks[type$$1].ToolbarElement;

    if (!blocks[type$$1].withChrome || ToolbarElement === undefined) {
      return null;
    }
...
const PopperRender = forwardRef((_ref2, _ref3) => {
  let scheduleUpdate = _ref2.scheduleUpdate,
      editorState = _ref2.editorState,
      style = _ref2.style,
      children = _ref2.children;
  let fragment = editorState.fragment;
...
var Toolbar = _ref4 => {
  let editorState = _ref4.editorState,
      blocks = _ref4.blocks,
      editor = _ref4.editor; // this element is created here so that when the popper rerenders
...
function getSchema(blocks) {
  ...
  Object.keys(blocks).forEach(type$$1 => {
    if (typeof blocks[type$$1].getSchema === 'function') {
      schema.blocks[type$$1] = blocks[type$$1].getSchema({
        blocks
      });
    }
  });
...
function Stories(_ref2) {
  ...
  let _useStateWithEquality = useStateWithEqualityCheck(null),
      ...
      editor = _useStateWithEquality2[0],
      setEditor = _useStateWithEquality2[1];
