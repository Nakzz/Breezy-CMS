...
var __chunk_1 = require('../../dist/chunk-8a043802.cjs.js');

var slate = require('slate');

var assert = _interopDefault(require('nanoassert')); // calling .toJSON() on all child nodes.
...
function serialize(value, blocks) {
  ...
  const serializedDocument = __chunk_1.walkSlateNode(value.document, {
    visitBlock(node) {
      ...
      const _block$serialize = block.serialize({
        value,
        node,
        blocks
      }),
            mutations = _block$serialize.mutations,
            serializedNode = _block$serialize.node;
...
function deserialize(_ref3, blocks) {
  let document = _ref3.document,
      serializations = _objectWithoutProperties(_ref3, ["document"]);
  ...
  return value.set('document', __chunk_1.walkSlateNode(value.document, {
    visitBlock(node) {
      const block = blocks[node.type]; // No matching block that we're in charge of
      ...
      const joins = (nodeData && nodeData.size && nodeData.get('_joinIds') || []).map(joinId => data.find(_ref4 => {
        let id = _ref4.id;
        return joinId === id;
