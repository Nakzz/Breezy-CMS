...
class MongooseMongoIdInterface extends adapterMongoose.MongooseFieldAdapter {
  ...
  setupHooks(_ref) {
    let addPreSaveHook = _ref.addPreSaveHook,
        addPostReadHook = _ref.addPostReadHook;
    if (!this.field.isPrimaryKey) return;
    addPreSaveHook(item => {
      if (item.id) {
        item._id = item.id;
...
const MongoId = {
  ...
  primaryKeyDefaults: {
    knex: {
      getConfig: () => {
        throw "The Uuid field type doesn't provide a default primary key field configuration for knex. " + "You'll need to supply your own 'id' field for each list or use a different field type for your " + "ids (eg '@keystone-alpha/fields-auto-increment').";
      }
    },
    mongoose: {
      getConfig: () => ({
